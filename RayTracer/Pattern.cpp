...
namespace RayTracer
{
    ...
    Ring::Ring(const Color &a, const Color &b)
        : Pattern(a,b) {}

    // ---------------------------------------------------------------------------------------

    Color Ring::At(const Point &p) const
    {
        if (int(floor(sqrt(pow(p.x,2.0) + pow(p.z,2.0)))) % 2 == 0)
            return first;
        else
            return second;
    }
    ...
    Color Checkers::At(const Point &p) const
    {
